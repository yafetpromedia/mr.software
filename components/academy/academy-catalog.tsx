...
import type { PublicAcademyCourse } from "@/lib/academy/courses";
...
const CARD_ACCENT = [
  ...
  "from-violet-500/20 to-indigo-400/5",
  "from-emerald-500/20 to-teal-400/5",
] as const;

export function AcademyCatalog({ courses }: { courses: PublicAcademyCourse[] }) {
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] w-full overflow-x-hidden border-t border-stone-200 bg-[var(--background)] dark:border-[var(--border)]">
      <div
        ...
        aria-hidden
      />
      <div className="pointer-events-none absolute inset-x-0 top-0 -z-10 h-72 bg-gradient-to-b from-orange-500/8 to-transparent dark:from-[var(--accent)]/10" aria-hidden />
      ...
      <div className="relative mx-auto w-full max-w-6xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
        <div className="max-w-2xl">
          ...
          <p className="mt-4 text-base leading-relaxed text-stone-600 dark:text-[var(--muted)]">
            Courses for developers and founders. Finish a track, then publish on the marketplace
            or open your storefront.
          </p>
        </div>
        ...
        {courses.length > 0 ? (
          <div className="mt-8 flex flex-wrap gap-3">
            {[
              ...
              <div
                key={stat.label}
                className="rounded-xl border border-stone-200 bg-white px-4 py-2.5 dark:border-[var(--border)] dark:bg-[var(--surface)]"
              >
            ...
            {courses.map((course, i) => (
              <li key={course.id}>
                <Link
                  ...
                  className="group flex h-full flex-col overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:border-orange-300 hover:shadow-lg dark:border-[var(--border)] dark:bg-[var(--surface)] dark:hover:border-[var(--accent)]/35"
                >
                  <div
                    className={`h-2 bg-gradient-to-r ${CARD_ACCENT[i % CARD_ACCENT.length]}`}
                    aria-hidden
                  />
                  <div className="flex flex-1 flex-col p-5 sm:p-6">
                    <span
                      className={`w-fit rounded-full px-2.5 py-0.5 text-[0.65rem] font-bold uppercase tracking-wide ${LEVEL_STYLE[course.level]}`}
                    >
                      {LEVEL_LABEL[course.level]}
                    </span>
                    <h2 className="mt-3 text-lg font-bold tracking-tight text-stone-900 transition group-hover:text-orange-600 dark:text-[var(--foreground)] dark:group-hover:text-[var(--accent)]">
                      {course.title}
                    </h2>
                    <p className="mt-2 line-clamp-3 flex-1 text-sm leading-relaxed text-stone-600 dark:text-[var(--muted)]">
                      {course.description}
                    </p>
                    <div className="mt-5 flex items-center justify-between gap-3 border-t border-stone-100 pt-4 dark:border-[var(--border)]">
                      <p className="text-xs font-medium text-stone-500 dark:text-[var(--muted)]">
                        {course.lessonCount} lessons · ~{course.durationMinutes} min
                      </p>
                      <span className="inline-flex items-center gap-1 text-xs font-semibold text-orange-600 opacity-0 transition group-hover:opacity-100 dark:text-[var(--accent)]">
                        Start
                        <svg className="h-3.5 w-3.5 transition group-hover:translate-x-0.5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" aria-hidden>
