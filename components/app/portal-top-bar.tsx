...
function PortalTopBarContent() {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const fromUrl = pathname.startsWith("/app/marketplace") ? (searchParams.get("q") ?? "") : "";
  ...
  const submit = useCallback(() => {
    const t = q.trim();
    if (t) {
      router.push(`/app/marketplace?q=${encodeURIComponent(t)}`);
    } else {
...
export function PortalTopBar() {
  ...
  if (!hydrated) {
    return (
      <TopBarSearchShell id="portal-search" label="Search software" placeholder="Search software…" />
    );
  }
  return (
    <Suspense
